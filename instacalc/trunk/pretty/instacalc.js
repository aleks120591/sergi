...
function replot(_4b, _4c) {
    ...
    try {
        ...
        for (i = 0; i < I.datatitles.length; i++) {
            _4f.push({
                label: I.datatitles[i][0],
                v: i + 1
            });
            _50.push([
                [i + 1], I.datatitles[i][1]]);
        }
        var _51 = (_4c == "bar" || _4c == "line") ? false : true;
        I.plotter = new EasyPlot(_4c, {
            "xTicks": _4f,
            "padding": {
                top: 5,
                bottom: 15,
            ...
            "colorScheme": PlotKit.Base.palette(PlotKit.Base.baseColors()[I.plotColorScheme]),
            "backgroundColor": PlotKit.Base.baseColors()[I.plotColorScheme].lighterColorWithLevel(0.45),
            "xOriginIsZero": _51,
            "pieRadius": 0.35
        },
        $("example2"), [_50]);
...
function hideInput(i) {
    ...
    var foo = new Effect.Fade(getInput(i), {
        duration: 0.1
    });
    foo = new Effect.Appear(getTitle(i), {
        duration: 0.2,
        scaleY: false
    });
}
...
function showInput(i) {
    ...
    hideObject(getTitle(i));
    var foo = new Effect.Appear(getInput(i), {
        duration: 0.2
    });
}
...
function extractComment(str) {
    ...
    if (!isNull(a)) {
        _cc = str.split(/[\/][\/]+/)[0];
        if (!isNull(_cc) && isdefined(_cc)) {} else {
            _cc = "";
        }
        var _cd = str.match("[/][/](.*)")[1];
        return [_cc, _cd];
